...
def list_books() -> List[str]:
    ...
    for name in sorted(os.listdir(OUTPUT_DIR)):
        final_path = os.path.join(OUTPUT_DIR, name, f"{name}_final.json")
...
def resolve_final_json(book_arg: str) -> Optional[str]:
    ...
    Accepts: a direct path to a *_final.json, or a book name whose extracted
    JSON lives at ``outputs/<book>/<book>_final.json``.
    """
    # 1) An explicit path to a final JSON.
    if book_arg.endswith(".json") and os.path.isfile(book_arg):
        return book_arg
    # 2) A book name -> outputs/<book>/<book>_final.json.
    candidate = os.path.join(OUTPUT_DIR, book_arg, f"{book_arg}_final.json")
    if os.path.isfile(candidate):
        return candidate
    # 3) A path to a book directory.
    if os.path.isdir(book_arg):
        base = os.path.basename(os.path.normpath(book_arg))
        candidate = os.path.join(book_arg, f"{base}_final.json")
        if os.path.isfile(candidate):
            return candidate
    return None
...
def generate_short_notes(
    ...
    *,
    ...
    Reads theory from ``final_json_path`` (never rewrites it), produces the rich
    study notes per topic via the existing summariser, and persists them through
    the existing sidecar exporter. Returns the merged study-notes path, or None.
    """
    with open(final_json_path, "r", encoding="utf-8") as handle:
        document = json.load(handle)

    book_slug = os.path.basename(final_json_path).replace("_final.json", "")
    topics = document.get("topics") or []
    print(f"Book: {book_slug}  ({len(topics)} topic(s) in {final_json_path})")
