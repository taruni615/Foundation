...
class DBConfig:
    ...
    NOTE: the DB layer does not read this — ``edu_pipeline.shared.db_config`` is
    the authority for live connections (see CLAUDE.md). Defaults are kept
    identical to it so the two never disagree if this section is adopted later.
    """
...
class ConfigService:
    ...
    def _load_from_env(cls) -> AppConfig:
        ...
        db_cfg = DBConfig(
            ...
            password=os.environ.get("DB_PASSWORD") or os.environ.get("MYSQL_PASSWORD") or "root",
            database=os.environ.get("DB_NAME") or os.environ.get("MYSQL_DATABASE") or "foundation",
        )
