...
class QuestionBankBuilder:
    ...
    def build_bank(
        ...
        valid_converted = []
        for q in converted_mcqs:
            ok, errs, clean = MCQValidator.validate(q, origin="converted")
            if ok:
                valid_converted.append(clean)

        valid_generated = []
        for q in generated_mcqs:
            ok, errs, clean = MCQValidator.validate(q, origin="generated")
            if ok:
                valid_generated.append(clean)
