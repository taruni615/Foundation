...
class RepositoryService:
    ...
    # Field names that have held section body text across format revisions.
    # v3.1 documents use "markdown"; the others are kept for older exports.
    THEORY_BODY_KEYS = ("markdown", "excerpt", "content", "text", "body")
    THEORY_HEADING_KEYS = ("heading", "title", "section_title", "name")

    @staticmethod
    def get_topic_theory_text(topic: Dict[str, Any]) -> str:
        """Return the readable theory text for one topic.

        Callers previously reached for ``excerpt``/``content``/``text`` only.
        Real v3.1 documents store section bodies under ``markdown``, so those
        callers silently produced a string of empty ``##`` headings and the
        notes model received no source material at all.
        """
        parts: List[str] = []
        for section in topic.get("theory_sections") or []:
            if not isinstance(section, dict):
                continue
            heading = next(
                (str(section[k]).strip() for k in RepositoryService.THEORY_HEADING_KEYS
                 if section.get(k)), "",
            )
            body = next(
                (str(section[k]).strip() for k in RepositoryService.THEORY_BODY_KEYS
                 if section.get(k)), "",
            )
            if not body:
                continue
        ...
        if not parts:
            for fallback in ("theory_notes", "theory_notes_preview", "source_topic_md"):
                value = str(topic.get(fallback) or "").strip()
                if value:
                    return value
        return "\n\n".join(parts).strip()
