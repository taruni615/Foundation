...
# Static assets moved into the package during the refactor; fall back to the
# legacy repo-root locations so pre-existing URLs keep resolving.
FRONTEND_DIR = PACKAGE_ROOT / "web" / "frontend" if (PACKAGE_ROOT / "web" / "frontend").is_dir() else PROJECT_ROOT
...
def resolve_repo_file(rel_path: str) -> Path:
    ...
    if rel.startswith("Viewer/"):
        alt_rel = rel.replace("Viewer/", "edu_pipeline/web/frontend/Viewer/")
        if (PROJECT_ROOT / alt_rel).is_file():
            rel = alt_rel
    elif rel.startswith("outputs/"):
        alt_rel = rel.replace("outputs/", "edu_pipeline/workspace/")
        if (PROJECT_ROOT / alt_rel).is_file():
            rel = alt_rel
