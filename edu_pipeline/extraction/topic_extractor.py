...
if not os.path.exists(os.path.dirname(DEFAULT_PDF_PATH)) and os.path.exists("Input_PDFs"):
    DEFAULT_PDF_PATH = os.path.join("Input_PDFs", "10 PHYSICS FOUNDATION.pdf")

# Credentials come from the environment / .env only — never hard-coded.
# See .env.example. Missing values surface as a clear error at OCR time.
MATHPIX_APP_ID = os.environ.get("MATHPIX_APP_ID", "")
MATHPIX_APP_KEY = os.environ.get("MATHPIX_APP_KEY", "")

MATHPIX_POLL_SECONDS = 2
MATHPIX_API_BASE = "https://api.mathpix.com/v3/pdf"
# Workspace / cache locations are owned by edu_pipeline.shared.paths and
# re-exported here so existing imports of these names keep working.
from edu_pipeline.shared.paths import (  # noqa: E402,F401
    MATHPIX_CACHE_DIR,
...
# Owned by edu_pipeline.shared.db_config; re-exported here so existing
# ``from topicwise_pipeline import DB_HOST`` imports keep resolving.
from edu_pipeline.shared.db_config import (  # noqa: E402,F401
    DB_CHARSET,
    DB_COLLATION,
    DB_HOST,
    DB_NAME,
...
def save_qa_table_json_sidecar(
    ...
    *,
    ...
    from edu_pipeline.storage.export_qa import save_qa_table_json_from_document
