...
# Labelled forms: "Assertion (A): ... Reason (R): ...", "Assertion: ... Reason: ...",
# or the standard instruction line naming both statements.
_ASSERTION_REASON_RE = re.compile(
    r"assertion\s*(?:\(\s*a\s*\)|[:\-])"          # "Assertion (A)" / "Assertion:"
...
def _plain(text: str) -> str:
    ...
    Entities are decoded first: MathML wraps fill-in-the-blank underscores as
    ``&#x0005F;``, so tag-stripping alone left them undetectable and those
    questions fell through to the wrong type.
    """
    if not text:
        return ""
    t = re.sub(r"<[^>]+>", " ", text)
    t = html.unescape(t)
    # MathML renders a blank as one <mi> per underscore, so tag-stripping leaves
    # "_ _ _ _". Collapse those runs back into a single blank token.
...
def classify_question(
    ...
    *,
    ...
    # 2. Assertion-Reason — require the labelled two-statement structure.
    # Matching on mere co-occurrence of both words mislabelled 36% of the bank.
